...
void ExecutionContext::ExecuteBlocks(absl::Span<ir::Addr const> ret_slots) {
  ...
  while (true) {
    ...
    switch (cmd_index) {
      ...
      default: {
        auto it = kInstructions.find(cmd_index);
        ASSERT(it != kInstructions.end());
