...
struct Parameters
  ...
  // Use `same_as` here so that implicit conversions prefer the `std::string`
  // overload.
  void append(std::same_as<std::string_view> auto name,
              parameter_value_type val,
              ParameterFlags flags = ParameterFlags()) {
    append(std::string(name), std::move(val), flags);
  }
