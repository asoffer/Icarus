...
Compiler::Task TaskForAt(ast::UnaryOperator const *node,
  ...
  QualifiedType qt;
  if (auto ptr_type =
          operand_qt.type().get_if<core::PointerType>(GlobalTypeSystem)) {
    qt = Reference(ptr_type->pointee());
  } else if (auto ptr_type =
                 operand_qt.type().get_if<BufferPointerType>(GlobalTypeSystem)) {
  ...
  switch (data.kind()) {
    ...
    default: {
      operand_task.send<FunctionData>(data);
      operand_task.complete();
      core::Bytes bytes_to_load = SizeOf(qt.type(), GlobalTypeSystem);
...
Compiler::Task TaskForAddress(ast::UnaryOperator const *node,
  ...
  if (operand_qualifiers >= Qualifiers::Buffer()) {
    qt = QualifiedType(BufferPointerType(GlobalTypeSystem, operand_qt.type()));
  } else if (operand_qualifiers >= Qualifiers::Reference()) {
    qt = QualifiedType(core::PointerType(GlobalTypeSystem, operand_qt.type()));
  } else {
...
Compiler::Task TaskForNegate(ast::UnaryOperator const *node,
  ...
  if (operand_type == Integer) {
    ...
    if (not found) { NTH_UNIMPLEMENTED("{}") <<= {DebugType(operand_type, GlobalTypeSystem)}; }
  }
}
