...
void ByteCodeValueEmitter::CastTo(ast::Expression const* node,
  ...
  if (from_qt.qualifiers() >= Qualifiers::Reference()) {
    if (auto p = to_qt.type().get_if<core::PointerType>(GlobalTypeSystem);
        p and p->pointee() == from_qt.type()) {
      as<ByteCodeReferenceEmitter>().Emit(node, data);
      return;
    } else if (auto p =
                   to_qt.type().get_if<BufferPointerType>(GlobalTypeSystem);
               p and p->pointee() == from_qt.type()) {
      as<ByteCodeReferenceEmitter>().Emit(node, data);
      return;
    }
  ...
  if (from_qt.type() == Integer) {
    ...
    if (to_qt.type() == I(8)) {
      ...
      NTH_UNIMPLEMENTED("{} {}") <<=
          {DebugType(to_qt.type()), DebugType(from_qt.type())};
    }
  } else if (auto from = from_qt.type().get_if<core::SizedIntegerType>(
                 GlobalTypeSystem)) {
    Emit(node, data);
    if (from_qt.type() == to_qt.type()) { return; }

    if (auto to =
            to_qt.type().get_if<core::SizedIntegerType>(GlobalTypeSystem)) {
      if (to->is_signed()) {
        if (from->is_signed()) {
    ...
    NTH_UNIMPLEMENTED("{} -> {}") <<=
        {DebugQualifiedType(from_qt),
         DebugQualifiedType(to_qt)};
  }
}
...
void ByteCodeStatementEmitter::operator()(ast::Cast const* node,
  ...
  if (IsNumeric(from_type) and IsNumeric(to_type)) {
    ...
    data.function().AppendDrop(SizeOf(to_type).value());
  }
}
