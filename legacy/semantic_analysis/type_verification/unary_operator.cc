  ...
  if (operand_qts.size() != 1) {
    ...
    switch (node->kind()) {
      ...
      case ast::UnaryOperator::Kind::At: {
        if (auto ptr_type =
                operand_type.get_if<core::PointerType>(GlobalTypeSystem)) {
          qt = Reference(ptr_type->pointee());
        } else if (auto ptr_type =
                       operand_type.get_if<BufferPointerType>(GlobalTypeSystem)) {
      ...
      case ast::UnaryOperator::Kind::Address: {
        if (operand_qualifiers >= Qualifiers::Buffer()) {
          qt = QualifiedType(BufferPointerType(GlobalTypeSystem, operand_type));
        } else if (operand_qualifiers >= Qualifiers::Reference()) {
          qt = QualifiedType(core::PointerType(GlobalTypeSystem, operand_type));
        } else {
