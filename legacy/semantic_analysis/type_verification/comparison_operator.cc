...
Relation Comparison(core::Type lhs, core::Type rhs) {
  // TODO: This implementation is incomplete and incorrect.
  if (IsIntegral(lhs)) {
    if (IsIntegral(rhs)) { return Relation::Ordered; }
    if (rhs.is<PrimitiveType>(GlobalTypeSystem)) { return Relation::Unordered; }
  } else if (auto lp = lhs.get_if<BufferPointerType>(GlobalTypeSystem)) {
    if (lhs == rhs) { return Relation::Ordered; }
    if (auto rp = rhs.get_if<core::PointerType>(GlobalTypeSystem)) {
      return lp->pointee() == rp->pointee() ? Relation::Comparable
                                            : Relation::Unordered;
    } else {
      return Relation::Unordered;
    }
  } else if (auto lp = lhs.get_if<core::PointerType>(GlobalTypeSystem)) {
    if (auto rp = rhs.get_if<BufferPointerType>(GlobalTypeSystem)) {
      return lp->pointee() == rp->pointee() ? Relation::Comparable
                                            : Relation::Unordered;
    } else if (auto rp = rhs.get_if<core::PointerType>(GlobalTypeSystem)) {
      return lp->pointee() == rp->pointee() ? Relation::Comparable
                                            : Relation::Unordered;
    } else {
      return Relation::Unordered;
    }
  } else if (lhs.is<PrimitiveType>(GlobalTypeSystem)) {
  ...
  } else if (auto le = lhs.get_if<EnumType>(GlobalTypeSystem)) {
...
VerificationTask TypeVerifier::VerifyType(ast::ComparisonOperator const *node) {
  ...
  for (size_t i = 0; i < node->ops().size(); ++i) {
    ...
    switch (Comparison(lhs_type, rhs_type)) {
