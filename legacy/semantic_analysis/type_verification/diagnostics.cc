...
struct StringifyType {
  ...
  explicit StringifyType(Context const *c, VisitationKind kind)
      : context_(*NTH_ASSERT_NOT_NULL(c)), kind_(kind) {}
  ...
  std::string operator()(ast::Cast const *node) {
    return ExpressionForDiagnostic(*node->type(), context_, GlobalTypeSystem);
  }
  ...
  std::string operator()(ast::Declaration::Id const *node) {
    ...
    if (auto *type_expr = node->declaration().type_expr()) {
      return ExpressionForDiagnostic(*type_expr, context_, GlobalTypeSystem);
    } else {
              ...
              absl::StrJoin(*node->outputs(), ", ",
                            [&](std::string *out, ast::Expression const *expr) {
                              out->append(ExpressionForDiagnostic(
                                  *expr, context_, GlobalTypeSystem));
                            });
  ...
  std::string operator()(ast::Terminal const *node) {
    ...
    if (auto p = t.get_if<PrimitiveType>(GlobalTypeSystem)) {
      ...
      return result;
    } else if (auto i = t.get_if<SliceType>(GlobalTypeSystem)) {
      return "[/]char";
    } else {
      NTH_UNIMPLEMENTED("{}") <<= {node->DebugString()};
    }
  ...
  std::string operator()(ast::UnaryOperator const *node) {
    ...
    switch (node->kind()) {
      ...
      case ast::UnaryOperator::Kind::Not: {
        core::Type t = context_.qualified_type(node).type();
        if (t.is<PrimitiveType>(GlobalTypeSystem) or
            t.is<core::SizedIntegerType>(GlobalTypeSystem)) {
          return (*this)(node->operand());
        } else {
          NTH_UNIMPLEMENTED("{}: {}") <<= {node->DebugString(), DebugType(t)};
        }
      } break;
    }
  }
...
std::string ExpressionForDiagnostic(ast::Expression const &expr,
  ...
  return StringifyExpression(&context)(&expr);
}
