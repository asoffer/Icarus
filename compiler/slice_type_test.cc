...
TEST(SliceType, Correct) {
  {
    ...
    EXPECT_THAT(snippet, EvaluatesTo<core::Type>(
                             SliceType(snippet.type_system(), I(64))));
  }
  {
    test::Snippet snippet(R"([/][/]i64)");
    ASSERT_THAT(snippet, AllOf(HasQualTypes(Constant(Type)), HasDiagnostics()));
    EXPECT_THAT(snippet, EvaluatesTo<core::Type>(SliceType(
                             snippet.type_system(),
                             SliceType(snippet.type_system(), I(64)))));
  }
  {
