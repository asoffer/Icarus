...
TEST(Match, OneParam) {
  ...
  {  // Call positionally with correct type
    ...
    EXPECT_EQ(matched_params.at(0).value.type(), type::Int32);
  }
...
TEST(Match, TwoParamsOneDefaulted) {
  ...
  {  // Call positionally with correct type
    ...
    EXPECT_EQ(matched_params.at(1).value.type(), type::Bool);
  }
...
TEST(Match, OneVariantParam) {
  ...
  {  // Call positionally with matching type
    ...
    EXPECT_EQ(matched_params.size(), 1);
    EXPECT_EQ(matched_params.at(0).name, "x");
    EXPECT_EQ(matched_params.at(0).value.type(), type::Bool);
  }
