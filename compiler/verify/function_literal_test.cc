...
TEST(FunctionLiteral, Trivial) {
  ...
  EXPECT_EQ(qt, type::QualType::Constant(type::Func({}, {})));
  EXPECT_THAT(mod.consumer.diagnostics(), IsEmpty());
}

TEST(FunctionLiteral, OneValidReturnType) {
  test::TestModule mod;
  type::QualType qt = mod.context().qual_type(
      mod.Append<ast::Expression>(R"(() -> i64 { return 3 })"));
  EXPECT_EQ(qt, type::QualType::Constant(type::Func({}, {type::I64})));
  EXPECT_THAT(mod.consumer.diagnostics(), IsEmpty());
}
...
TEST(FunctionLiteral, MultipleValidReturnTypes) {
  ...
  EXPECT_EQ(qt,
            type::QualType::Constant(type::Func({}, {type::I64, type::Bool})));
  EXPECT_THAT(mod.consumer.diagnostics(), IsEmpty());
}
...
TEST(FunctionLiteral, MultipleParametersOneReturnType) {
  test::TestModule mod;
  type::QualType qt = mod.context().qual_type(mod.Append<ast::Expression>(
...
TEST(FunctionLiteral, ReturnTypeAllowsCasting) {
  ...
  EXPECT_EQ(qt, type::QualType::Constant(
                    type::Func({core::Param("p", type::QualType::NonConstant(
