...
#include "ir/byte_code/context.h"
...
struct Context {
  ...
  std::pair<ir::NativeFn, bool> add_func(
    ...
    if (inserted) { entry = ir::NativeFn(InsertFunction(fn_type)); }
  ...
  std::pair<ir::Jump, bool> add_jump(ast::Jump const *expr) {
    ...
    auto [iter, inserted]       = ir_jumps_.try_emplace(expr, jump_type);
  ...
  void WriteByteCode(ir::NativeFn f) {
    fn_data_.at(f)->byte_code = byte_code_context_.emplace(f, EmitByteCode(*f));
  }
 ...
 private:
  ...
  ir::NativeFn::Data const *InsertFunction(type::Function const *fn_type);
  ...
  ir::ByteCodeContext byte_code_context_;
