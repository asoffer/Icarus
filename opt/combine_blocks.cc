...
// }
//
// // Constructs a map from a block index to a pair. The first element of the
// pair
// // is the number of incoming blocks. If that number is 1 and the unique
// incoming
...
//     processing.pop();
//     if (not alive.insert(current).second) { continue; }
...
void CombineBlocks(ir::CompiledFn* fn) {
  ...
  //   } DeleteDeadBlocks(fn); auto incoming = IncomingMap(fn);
  //
  //   {
  ...
  //       --tail.value;
  //     } else if (auto& head_block = fn->block(head); not
  //     head_block.cmds_.empty()) {
  //       ++head.value;
  //     } else {
