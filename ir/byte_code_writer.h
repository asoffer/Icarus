...
void ReadInto(T& ref, base::untyped_buffer::const_iterator* iter) {
  if constexpr (
      base::meta<T>.template inherits_from<ReadFromByteCodeExtension<T>>()) {
    ref = T::ReadFromByteCode(iter);
  ...
  } else if constexpr (base::meta<T>.template is_a<absl::flat_hash_set>()) {
    ASSERT(ref.size() == 0u);
    uint16_t num_entries = iter->read<uint16_t>();
    ref.reserve(num_entries);
    for (uint16_t i = 0; i < num_entries; ++i) {
  ...
  } else if constexpr (base::meta<T> == base::meta<base::untyped_buffer>) {
    ASSERT(ref.size() == 0u);
    uint16_t num_bytes = iter->read<uint16_t>();
    ref.write(0, iter->raw(), num_bytes);
    iter->skip(num_bytes);
  } else {
    ref = iter->read<T>();
  }
...
struct ByteCodeWriter {
  ...
  void Write(T const& val) {
    if constexpr (
        base::meta<T>.template inherits_from<WriteByteCodeExtension<T>>()) {
      val.WriteByteCode(this);
    } else if constexpr (base::meta<T>.template is_a<std::vector>()) {
...
template <typename T>
void WriteByteCodeExtension<T>::WriteByteCode(ByteCodeWriter* writer) const {
  std::apply([&](auto const&... field) { (writer->Write(field), ...); },
             static_cast<T const*>(this)->field_refs());
}
template <typename T>
T ReadFromByteCodeExtension<T>::ReadFromByteCode(
