...
base::untyped_buffer Execute(IR::Func *fn,
  ...
  while (true) {
    ...
    if (block_index.is_default()) {
      ...
      // TODO we should actually be able to calculate the required size and
      // not need a guess. Just move forward to alignment over all the
      // outputs. But this is probably worth storing somewhere on the function
      // so thing about the right way to do it.
  ...
  BlockIndex ExecContext::ExecuteCmd(const Cmd &cmd) {
    ...
    switch (cmd.op_code_) {
      ...
      case Op::Extend:
        ...
        break;
      case Op::Bytes:
        save(Architecture::InterprettingMachine().bytes(
            resolve(cmd.bytes_.arg_)));
        break;
      case Op::Align:
      ...
      case Op::LoadReal:
        ...
        break;
      case Op::LoadType:
        save(LoadValue<type::Type const *>(resolve(cmd.load_type_.arg_),
                                           stack_));
        break;
      case Op::LoadEnum:
      ...
      case Op::EqReal:
        ...
        break;
      case Op::EqFlags:
        save(resolve(cmd.eq_flags_.args_[0]) ==
             resolve(cmd.eq_flags_.args_[1]));
        break;
      case Op::EqType:
      ...
      case Op::NeReal:
        ...
        break;
      case Op::NeFlags:
        save(resolve(cmd.ne_flags_.args_[0]) ==
             resolve(cmd.ne_flags_.args_[1]));
        break;
      case Op::NeType:
      ...
      case Op::VariantValue: {
        auto bytes =
            Architecture::InterprettingMachine().bytes(Ptr(type::Type_));
        auto bytes_fwd =
        ...
        switch (addr.kind) {
          ...
          case Addr::Kind::Heap:
            addr.as_heap = static_cast<void *>(
                static_cast<char *>(addr.as_heap) + bytes_fwd);
            save(addr);
            break;
      ...
      case Op::Call: {
        ...
        for (size_t i = 0; i < cmd.call_.long_args_->is_reg_.size(); ++i) {
          ...
          auto *t     = cmd.call_.long_args_->types_->at(i);
          ...
          if (t == type::Bool) {
            ...
            call_buf.append(is_reg
                                ? resolve<i32>(long_args.get<Register>(offset))
                                : long_args.get<i32>(offset));
          } else if (t == type::Real) {
            call_buf.append(
                is_reg ? resolve<double>(long_args.get<Register>(offset))
