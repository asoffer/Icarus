...
TEST(ArrayLiteral, OneElement) {
  ...
  EXPECT_THAT(
      repl.type_check(R"([0])"),
      AllOf(HasQualTypes(Constant(ArrayType(repl.type_system(), 1, Integer))),
            HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"([true])"),
      AllOf(HasQualTypes(Constant(ArrayType(repl.type_system(), 1, Bool))),
            HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"([[true]])"),
      AllOf(
          HasQualTypes(Constant(ArrayType(
              repl.type_system(), 1, ArrayType(repl.type_system(), 1, Bool)))),
          HasDiagnostics()));
  EXPECT_THAT(repl.type_check(R"([[[]]])"),
              AllOf(HasQualTypes(Constant(ArrayType(
                        repl.type_system(), 1,
                        ArrayType(repl.type_system(), 1, EmptyArray)))),
                    HasDiagnostics()));
}
...
TEST(ArrayLiteral, MultipleMatchingElements) {
  ...
  EXPECT_THAT(
      ...
      AllOf(HasQualTypes(Constant(ArrayType(repl.type_system(), 2, Integer))),
            HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"(
      a: bool
      ...
      AllOf(HasQualTypes(QualifiedType(ArrayType(repl.type_system(), 2, Bool))),
            HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"([true, false])"),
      AllOf(HasQualTypes(Constant(ArrayType(repl.type_system(), 2, Bool))),
            HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"([[true], [false]])"),
      AllOf(
          HasQualTypes(Constant(ArrayType(
              repl.type_system(), 2, ArrayType(repl.type_system(), 1, Bool)))),
          HasDiagnostics()));
  EXPECT_THAT(
      repl.type_check(R"(
      b: bool
      ...
      AllOf(
          HasQualTypes(QualifiedType(ArrayType(
              repl.type_system(), 2, ArrayType(repl.type_system(), 1, Bool)))),
          HasDiagnostics()));
  EXPECT_THAT(repl.type_check(R"([[[]], [[]], [[]]])"),
              AllOf(HasQualTypes(Constant(ArrayType(
...
TEST(ArrayLiteral, Recovery) {
  ...
  EXPECT_THAT(
      ...
      AllOf(
          HasQualTypes(Error(Constant(ArrayType(repl.type_system(), 5, Bool)))),
          HasDiagnostics(
