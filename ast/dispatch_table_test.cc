...
TEST_CASE("() -> ()") {
  ...
  SECTION("without args") {
    ...
    CHECK(VerifyDispatch(&mod.compiler, call_expr, os,
                         ResultsForArgs(&mod.compiler, call_expr->args())) ==
...
TEST_CASE("overload set") {
  ...
  SECTION("without variant - matching one") {
    mod.Process(test::ParseAs<Declaration>("v: bool | float32 = true "));
  ...
  SECTION("without variant - matching both") {
    ...
    Call call_expr(frontend::SourceRange{}, test::ParseAs<Expression>("f"),
                   test::MakeFnArgs({"v"}, {}));
    CHECK(
        VerifyDispatch(&mod.compiler, &call_expr, os,
                       ResultsForArgs(&mod.compiler, call_expr.args())) ==
