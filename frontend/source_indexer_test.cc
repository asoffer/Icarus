...
TEST(SourceIndexer, Insert) {
  ...
  std::string_view content1 = indexer.insert(ir::ModuleId(0), R"(abc
  def
  ghi)");
  ...
  std::string_view content2 = indexer.insert(ir::ModuleId(0), "");
  EXPECT_EQ(content2, content1);

  std::string_view content3 = indexer.insert(ir::ModuleId(1), "abc");
  EXPECT_EQ(content3, "abc");
}

TEST(SourceIndexer, EntryFor) {
  SourceIndexer indexer;
  std::string_view content = indexer.insert(ir::ModuleId(0), R"(abc
  def
  ghi)");
